...
function negate($im)
{
    ...
    for($x = 0; $x < imagesx($im); ++$x)
    {
     ...
     $color = imagecolorallocate($im, intval(10*pow($rgb['red'], 0.55)), intval(10*pow($rgb['green'],0.55)), intval(10*pow($rgb['blue'],0.55)));
...
if($im && negate($im))
{
   // echo 'Image successfully converted to negative colors.';
    ...
    imagejpeg($im, 'watermark/p'.$name, 100);
    imagedestroy($im);
    echo '<img src="watermark/p'.$name.'" height="300" width="300" />';
}
else
{
