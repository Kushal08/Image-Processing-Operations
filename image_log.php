...
$c = 40;
    for($x = 0; $x < imagesx($im); ++$x)
    {
        for($y = 0; $y < imagesy($im); ++$y)
        {
            ...
            $color = imagecolorallocate($im, intval($c*log($rgb['red']+1)), intval($c*log($rgb['green']+1)), intval($c*log($rgb['blue']+1)));
...
if($im && negate($im))
{
    ...
    imagejpeg($im, 'watermark/l'.$name, 100);
    imagedestroy($im);
    echo '<img src="watermark/l'.$name.'" height="300" width="300" />';
}
else
{
